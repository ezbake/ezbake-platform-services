...
public class EzGroupsGraphTest extends GraphCommonSetup {
    ...
    public void createChildPathExistsTest() throws VertexExistsException, InvalidVertexTypeException, UserNotFoundException, AccessDeniedException, IndexUnavailableException, InvalidGroupNameException {
        ...
        Assert.assertTrue(query.getBaseQuery().pathExists(user1, g2id, BaseEdge.EdgeType.A_CREATE_CHILD.toString()));

        // User2 can create child on group that inherited, but not on group that did not inherit
        Assert.assertTrue(query.getBaseQuery().pathExists(user2, g1id, BaseEdge.EdgeType.A_CREATE_CHILD.toString()));
        Assert.assertTrue(!query.getBaseQuery().pathExists(user2, g2id, BaseEdge.EdgeType.A_CREATE_CHILD.toString()));

        // User2 can create child on group off common that did not inherit, but User1 cannot
        Assert.assertTrue(query.getBaseQuery().pathExists(user2, g4id, BaseEdge.EdgeType.A_CREATE_CHILD.toString()));
        Assert.assertTrue(!query.getBaseQuery().pathExists(user1, g4id, BaseEdge.EdgeType.A_CREATE_CHILD.toString()));
    }
    ...
    public void appUserGroupsList() throws VertexExistsException, InvalidVertexTypeException, UserNotFoundException, AccessDeniedException, IndexUnavailableException, InvalidGroupNameException {
        ...
        ImmutableSet<String> groupNames = new ImmutableSortedSet.Builder<String>(Ordering.natural())
                .add(Group.COMMON_GROUP)
                .add(Joiner.on(EzGroupsGraphImpl.GROUP_NAME_SEP).join(EzGroupsConstants.ROOT, EzGroupsConstants.APP_ACCESS_GROUP))
                    .add(Group.COMMON_GROUP + ".group1")
                    .add(Group.COMMON_GROUP + ".group1.group2")
                    .add(Group.COMMON_GROUP + ".group1.group2.group3")
                    .add(Group.COMMON_GROUP + ".group4")
                    .add(Joiner.on(EzGroupsGraphImpl.GROUP_NAME_SEP).join(Group.COMMON_GROUP, Group.APP_ACCESS_GROUP, "App User One"))
                    .add(Joiner.on(EzGroupsGraphImpl.GROUP_NAME_SEP).join(Group.COMMON_GROUP, Group.APP_GROUP, "App User One"))
                    .build();
    ...
    public void removeUserFromGroupTest() throws VertexExistsException, UserNotFoundException, InvalidVertexTypeException, AccessDeniedException, IndexUnavailableException, InvalidGroupNameException {
        ...
        Assert.assertTrue(
                !new GremlinPipeline(user2.asVertex()).outE(BaseEdge.EdgeType.DATA_ACCESS.toString()).inV()
                        .has("id", g1id).hasNext());
    }
    ...
    @Test
    public void testModifyUserPrincipalToAlreadyExisting()
            throws VertexExistsException, IndexUnavailableException, InvalidVertexTypeException, UserNotFoundException,
            AccessDeniedException, InvalidGroupNameException, VertexNotFoundException {

        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, USER1_ID).asVertex().getId();
        graph.addUser(BaseVertex.VertexType.USER, USER2_ID, USER2_ID).asVertex().getId();

        try {
            graph.updateUser(BaseVertex.VertexType.USER, USER1_ID, USER2_ID);
            Assert.fail(EXCEPTION_FAIL_MSG);
    ...
    public void testEdgesBetweenVertices() {
        ...
        Set<BaseEdge.EdgeType> edges = graph.edgesBetweenVertices(
                v1, v3, BaseEdge.EdgeType.A_CREATE_CHILD, BaseEdge.EdgeType.A_READ, BaseEdge.EdgeType.A_WRITE,
                BaseEdge.EdgeType.DATA_ACCESS, BaseEdge.EdgeType.A_MANAGE);

        ImmutableSet<BaseEdge.EdgeType> expectedEdges = ImmutableSortedSet.of(
                BaseEdge.EdgeType.A_CREATE_CHILD, BaseEdge.EdgeType.A_WRITE, BaseEdge.EdgeType.DATA_ACCESS);

        Assert.assertEquals(expectedEdges, ImmutableSortedSet.copyOf(edges));
    }

    @Test
    public void testGetUsersRejectsNonUserType() throws Exception {
        try {
            graph.getUsers(BaseVertex.VertexType.APP_GROUP, Sets.newHashSet("abc"));
            Assert.fail(EXCEPTION_FAIL_MSG);
        ...
        try {
            graph.getUsers(BaseVertex.VertexType.GROUP, Sets.newHashSet("abc"));
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (IllegalArgumentException e) {
            // exception thrown as expected
        }
        ...
        try {
            graph.getUsers(BaseVertex.VertexType.GROUP_MAPPING, Sets.newHashSet("abc"));
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (IllegalArgumentException e) {
            // exception thrown as expected
        }
    ...
    /**
     * Testing getting users by EzBake ID.
     */
    @Test
    public void testGetUsers()
            throws VertexExistsException, IndexUnavailableException, InvalidVertexTypeException, UserNotFoundException,
            AccessDeniedException, InvalidGroupNameException {
        final User user1 = graph.addUser(BaseVertex.VertexType.USER, USER1_ID,USER1_ID);
        final User user2 = graph.addUser(BaseVertex.VertexType.USER, USER2_ID, USER2_ID);

        final User appUser1 = graph.addUser(BaseVertex.VertexType.APP_USER, USER1_ID, USER1_ID);
        final User appUser2 = graph.addUser(BaseVertex.VertexType.APP_USER, USER2_ID, USER2_ID);

        final Set<User> expectedUsers = Sets.newHashSet(user1,user2);
        final Set<User> expectedAppUsers = Sets.newHashSet(appUser1, appUser2);

        final Set<User> actualUsers = graph.getUsers(BaseVertex.VertexType.USER, Sets.newHashSet(USER1_ID, USER2_ID));
        final Set<User> actualAppUsers = graph.getUsers(
                BaseVertex.VertexType.APP_USER, Sets.newHashSet(USER1_ID, USER2_ID));

        Assert.assertNotEquals(actualUsers,actualAppUsers);
        Assert.assertEquals(expectedUsers, actualUsers);
        Assert.assertEquals(expectedAppUsers, actualAppUsers);

        final Set<User> singleUserResult = graph.getUsers(BaseVertex.VertexType.USER, Sets.newHashSet(USER1_ID));
        Assert.assertEquals(Sets.newHashSet(user1), singleUserResult);
    }
    ...
    public void testGetGroups() throws InvalidVertexTypeException, AccessDeniedException, IndexUnavailableException, UserNotFoundException, VertexExistsException, VertexNotFoundException, InvalidGroupNameException {
        ...
        Group g1 = graph.addGroup(
                type, id, groupName1, new GroupInheritancePermissions(true, true, false, false, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
        Group g2 = graph.addGroup(
                type, id, groupName2, new GroupInheritancePermissions(true, true, false, false, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
        Group g3 = graph.addGroup(
                type, id, groupName3, new GroupInheritancePermissions(true, false, false, false, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
    ...
    public void testChildGroupsNestedLower() throws InvalidVertexTypeException, AccessDeniedException, UserNotFoundException, VertexExistsException, VertexNotFoundException, IndexUnavailableException, InvalidGroupNameException {
        ...
        Group g1 = graph.addGroup(
                ...
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
        Group g2 = graph.addGroup(
                type, id, groupName2, expectedGroup1, new GroupInheritancePermissions(true, true, false, false, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
...
//        }
    ...
    @Test
    public void testGetGroupsById() throws Exception {
        final Long user1Index = graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One").getIndex();

        final Group group1 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        final Group group2 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP123_FRIENDLY_NAME, GROUP12_FQNAME);

        final Group group4 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP13_FRIENDLY_NAME, GROUP1_FQNAME);

        final long index1 = group1.getIndex();
        final long index2 = group2.getIndex();
        final long index4 = group4.getIndex();

        // user1Index should be ignored
        final Set<Long> indices = Sets.newHashSet(index1, index2, index4, user1Index);
        final Set<Group> groups = Sets.newHashSet(group1, group2, group4);

        Assert.assertEquals(graph.getGroupsByIds(indices), groups);
    }

    /**
     * Tests {@code getGroupsByIdsWithAuths(...)}.  Also see {@link ezbake.groups.graph.PermissionEnforcerTest}, which
     * is currently used to determine if an user has any permission.
     */
    @Test
    public void testGetGroupsByIdWithAuths() throws Exception {
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        final Long user2Index = graph.addUser(BaseVertex.VertexType.USER, USER2_ID, "User Two").getIndex();

        final Group group1 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);

        // Assert owner has any permission
        Assert.assertEquals(
                Sets.newHashSet(group1), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER1_ID, Sets.newHashSet(group1.getIndex())));

        // Assert that user2 does not have any permission
        Assert.assertEquals(
                Sets.newHashSet(), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER2_ID, Sets.newHashSet(group1.getIndex())));

        // give user2 a permission
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, false, false, false, false, true);

        // Assert that after adding user2 with a single permission, they are determined to have any access to the group.
        Assert.assertEquals(
                Sets.newHashSet(group1), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER2_ID, Sets.newHashSet(group1.getIndex())));

        // reset user 2's auths, check they are gone
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, false, false, false, false, false);

        Assert.assertEquals(
                Sets.newHashSet(), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER2_ID, Sets.newHashSet(group1.getIndex())));

        // give user2 data_access which may be categorized differently than other permissions
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, true, false, false, false, false);

        Assert.assertEquals(
                Sets.newHashSet(group1), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER2_ID, Sets.newHashSet(group1.getIndex())));

        // add a child group that inherits data access and check that we have permission on both
        final Group group2 = graph.addGroup(
        ...
        // also throwing in user2Index to the requests to insure it is ignored
        Assert.assertEquals(
                Sets.newHashSet(group1, group2), graph.getGroupsByIdsWithAuths(
                        BaseVertex.VertexType.USER, USER2_ID,
                        Sets.newHashSet(group1.getIndex(), group2.getIndex(), user2Index)));
    }

    @Test
    public void testGetAllGroups()
            throws UserNotFoundException, AccessDeniedException, IndexUnavailableException, InvalidGroupNameException,
            VertexExistsException, VertexNotFoundException, InvalidVertexTypeException {
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One").asVertex();
        final Group group1 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        final Group group2 = graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME);
        final Group group3 =
                graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP123_FRIENDLY_NAME, GROUP12_FQNAME);
        ...
        final Group app = graph.getGroup("root.app");

        final Set<Group> expectedGroups = Sets.newHashSet(group1, group2, group3, group4,app);
    ...
    public void testChangeGroupNameToAlreadyExisting()
            throws UserNotFoundException, AccessDeniedException, IndexUnavailableException, InvalidGroupNameException,
            VertexExistsException, InvalidVertexTypeException, VertexNotFoundException {
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One").asVertex();
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, "someName");

        try {
            graph.changeGroupName(BaseVertex.VertexType.USER, USER1_ID, "root.someName", GROUP1_FRIENDLY_NAME);
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (VertexExistsException e) {
            // expected exception
        }
    ...
    public void testChangeGroupNameNoChildren() throws Exception {
        ...
        // Change group name
        final Map<String, String> changed = graph.changeGroupName(GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);

        // Verify expected changed names are included in return value
        Map<String, String> expectedChangedNames = Maps.newHashMap();
        expectedChangedNames.put(GROUP1_FQNAME, RENAMED_FQ_GROUP1NAME);
        Assert.assertEquals(expectedChangedNames, changed);

        // Verify group renamed
        graph.getGroup(RENAMED_FQ_GROUP1NAME);

        // Verify group does not exist with original name
        try {
            graph.getGroup(GROUP1_FQNAME);
            Assert.fail(EXCEPTION_FAIL_MSG);
    ...
    public void testChangeGroupNameOneChild() throws Exception {
        ...
        // Add group with a child using user
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME);

        // Change group name
        final Map<String, String> changed = graph.changeGroupName(GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);

        // Verify expected changed names are included in return value
        final Map<String, String> expectedChangedNames = Maps.newHashMap();
        expectedChangedNames.put(GROUP1_FQNAME, RENAMED_FQ_GROUP1NAME);
        expectedChangedNames.put(GROUP12_FQNAME, RENAMED_FQ_GROUP12NAME);
        Assert.assertEquals(expectedChangedNames, changed);

        // Verify renamed groups exist
        final Group g1p = graph.getGroup(RENAMED_FQ_GROUP1NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP1NAME, g1p.getGroupName());
        Assert.assertEquals(FRIENDLY_GROUPNAME_TO_RENAME_WITH, g1p.getGroupFriendlyName());

        final Group g2p = graph.getGroup(RENAMED_FQ_GROUP12NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP12NAME, g2p.getGroupName());
        Assert.assertEquals(GROUP12_FRIENDLY_NAME, g2p.getGroupFriendlyName());

        // Verify groups do not exist by original names
        try {
            graph.getGroup(GROUP1_FQNAME);
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (final VertexNotFoundException e) {
    ...
    public void testChangeGroupNameMultChild() throws Exception {
        ...
        // Add multiple levels of child groups with user
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP123_FRIENDLY_NAME, GROUP12_FQNAME);
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP13_FRIENDLY_NAME, GROUP1_FQNAME);
        ...
        expectedChangedNames.put(GROUP1_FQNAME, RENAMED_FQ_GROUP1NAME);
        expectedChangedNames.put(GROUP12_FQNAME, RENAMED_FQ_GROUP12NAME);
        expectedChangedNames.put(GROUP123_FQNAME, RENAMED_FQ_GROUP123NAME);
        expectedChangedNames.put(GROUP13_FQNAME, RENAMED_FQ_GROUP13NAME);
        Assert.assertEquals(expectedChangedNames, changed);

        // Verify group names changed
        final Group g1p = graph.getGroup(RENAMED_FQ_GROUP1NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP1NAME, g1p.getGroupName());
        Assert.assertEquals(FRIENDLY_GROUPNAME_TO_RENAME_WITH, g1p.getGroupFriendlyName());
        ...
        final Group g3p = graph.getGroup(RENAMED_FQ_GROUP123NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP123NAME, g3p.getGroupName());
        Assert.assertEquals(GROUP123_FRIENDLY_NAME, g3p.getGroupFriendlyName());

        final Group g4p = graph.getGroup(RENAMED_FQ_GROUP13NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP13NAME, g4p.getGroupName());
        Assert.assertEquals(GROUP13_FRIENDLY_NAME, g4p.getGroupFriendlyName());
        ...
        try {
            graph.getGroup(GROUP123_FQNAME);
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (final VertexNotFoundException e) {
            // exception thrown as expected
        }
    ...
    @Test
    public void testChangeGroupNameWithAuthsRejectsNonUser() throws Exception {
        try {
            graph.changeGroupName(BaseVertex.VertexType.APP_GROUP, "blah", "blah", "blah");
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (IllegalArgumentException e) {
            // exception thrown as expected
        }
        ...
        try {
            graph.changeGroupName(BaseVertex.VertexType.GROUP, "blah", "blah", "blah");
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (IllegalArgumentException e) {
            // exception thrown as expected
        }
        ...
        try {
            graph.changeGroupName(BaseVertex.VertexType.GROUP_MAPPING, "blah", "blah", "blah");
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (IllegalArgumentException e) {
            // exception thrown as expected
        }
    ...
    @Test
    public void testChangeGroupNameWithAuthsNoChildren() throws Exception {
        // Add user
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        ...
        // Change group name
        final Map<String, String> changed = graph.changeGroupName(
                BaseVertex.VertexType.USER, USER1_ID, GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);
        ...
        // Verify group does not exist by original name
        try {
            graph.getGroup(GROUP1_FQNAME);
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (final VertexNotFoundException e) {
    ...
    @Test
    public void testChangeGroupNameWithAuthsNoChildrenDenied() throws Exception {
        // Add users
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        graph.addUser(BaseVertex.VertexType.USER, USER2_ID, "User Two");

        // Add group that user1 owns but has no relationship with user2
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);

        // Access should be denied to user2:
        try {
            graph.changeGroupName(
                    BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);
            Assert.fail(EXCEPTION_FAIL_MSG);
    ...
    @Test
    public void testChangeGroupNameWithAuthsOneChild() throws Exception {
        // Add users
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        graph.addUser(BaseVertex.VertexType.USER, USER2_ID, "User Two");
        ...
        // Add user2 so we can add groups that user1 does not own by default
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, true, true, true, true, true);

        // group12 inherits admin manage permission from group , so user1 should be able to manage this group
        graph.addGroup(
                BaseVertex.VertexType.USER, USER2_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME,
                new GroupInheritancePermissions(false, false, false, true, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
    ...
    @Test
    public void testChangeGroupNameWithAuthsOneChildDenied() throws Exception {
        // Add users
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        graph.addUser(BaseVertex.VertexType.USER, USER2_ID, "User Two");

        // Add group1 that user1 will own
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);

        // Add user2 so we can add groups that user1 does not own by default
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, true, true, true, true, true);

        // user2 adds group12 that does not inherit admin manage. user1 should NOT have access by default
        graph.addGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME);

        // group12 doesn't inherit permission from group1, so user 1 should NOT have access to the below requests:
        try {
            graph.changeGroupName(
                    BaseVertex.VertexType.USER, USER1_ID, GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);
            Assert.fail(EXCEPTION_FAIL_MSG);
        ...
        try {
            graph.changeGroupName(
                    BaseVertex.VertexType.USER, USER1_ID, GROUP12_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);
            Assert.fail(EXCEPTION_FAIL_MSG);
        } catch (final AccessDeniedException e) {
            // exception thrown as expected
        }
    ...
    @Test
    public void testChangeGroupNameMultChildWithAuths() throws Exception {
        // Add an user
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        ...
        // Validate groups renamed as expected
        final Group g1p = graph.getGroup(RENAMED_FQ_GROUP1NAME);
        Assert.assertEquals(RENAMED_FQ_GROUP1NAME, g1p.getGroupName());
        Assert.assertEquals(FRIENDLY_GROUPNAME_TO_RENAME_WITH, g1p.getGroupFriendlyName());
    ...
    @Test
    public void testChangeGroupNameMultChildWithAuthsDenied() throws Exception {
        // Add users
        graph.addUser(BaseVertex.VertexType.USER, USER1_ID, "User One");
        graph.addUser(BaseVertex.VertexType.USER, USER2_ID, "User Two");

        // Create new group and add user2 to it so that we can create groups without giving ownership to user1
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP1_FRIENDLY_NAME);
        graph.addUserToGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP1_FQNAME, true, true, true, true, true);

        // Inherits admin manage
        graph.addGroup(
                BaseVertex.VertexType.USER, USER2_ID, GROUP12_FRIENDLY_NAME, GROUP1_FQNAME,
                new GroupInheritancePermissions(false, false, false, true, false),
                new UserGroupPermissionsWrapper(true, true, true, true, true), true, false);
        ...
        // Doesn't inherit
        graph.addGroup(BaseVertex.VertexType.USER, USER2_ID, GROUP123_FRIENDLY_NAME, GROUP12_FQNAME);

        // Not directly involved in test, but here for consistency with other multi-child tests
        graph.addGroup(BaseVertex.VertexType.USER, USER1_ID, GROUP13_FRIENDLY_NAME, GROUP1_FQNAME);

        // group12 inherits, from a group user1 has access to, but group123 (inherits from group12) does not
        // Access should be denied for name change request by user1 on any ancestors of group123
        try {
            graph.changeGroupName(
                    BaseVertex.VertexType.USER, USER1_ID, GROUP1_FQNAME, FRIENDLY_GROUPNAME_TO_RENAME_WITH);
            Assert.fail(EXCEPTION_FAIL_MSG);
