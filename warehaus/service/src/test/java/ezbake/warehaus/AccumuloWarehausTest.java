...
//	@Test
    ...
    @Test
    public void viewWithSquashTest() throws TException, InterruptedException {
        //Test inserting an intial view with squash turned on
        ViewId id = new ViewId("view_test_squash_uri","testcases","myview").setSquashPrevious(true);
        long ts1 = warehaus.insertView(ByteBuffer.wrap("show me this".getBytes()), id, visibility, token).getTimestamp();
        Thread.sleep(1000);
        BinaryReplay val1 = warehaus.getView(id, ts1, token);
        assertEquals("show me this", new String(val1.getPacket()));
    ...
    @Test
    public void updateExistingViewTest() throws TException, InterruptedException {
        ViewId id = new ViewId("view_test_uri_existing","testcases","myview");
        long ts1 = warehaus.insertView(ByteBuffer.wrap("show me this".getBytes()), id, visibility, token).getTimestamp();
        Thread.sleep(1000);
        id.setTimestamp(TimeUtil.convertToThriftDateTime(ts1));
        long ts2 = warehaus.insertView(ByteBuffer.wrap("show me that instead".getBytes()), id, visibility, token).getTimestamp();
        assertEquals(ts1, ts2);
        BinaryReplay val1 = warehaus.getView(id, ts1, token);
        BinaryReplay val2 = warehaus.getLatestView(id, token);
        assertEquals(new String(val2.getPacket()), new String(val1.getPacket()));
    }

    @Test
    public void squashExistingViewTest() throws TException, InterruptedException {
        ViewId id = new ViewId("view_test_uri_existing_squash","testcases","myview");
        //Let's add two views to the warehouse
        Repository data = new Repository("view_test_uri_existing_squash", ByteBuffer.wrap("This is my raw".getBytes()),
                ByteBuffer.wrap("This is my parsed".getBytes()));
        long ts = warehaus.insert(data, visibility, token).getTimestamp();
        Thread.sleep(1000);
        warehaus.insertView(ByteBuffer.wrap("show me this 1".getBytes()), id, visibility, token);
        Thread.sleep(1000);
        warehaus.insertView(ByteBuffer.wrap("show me this 2".getBytes()), id, visibility, token);
        Thread.sleep(1000);
        List<Long> timestamps =  warehaus.getVersions(id.getUri(), token);
        //Assert that we get our two views back plus parsed and raw
        assertEquals(3, timestamps.size());


        Set<String> expectedStrings = Sets.newHashSet("show me this 1", "show me this 2");
        Set<String> actualStrings = new HashSet<>();
        for(Long timestamp : timestamps) {
            if (timestamp != ts) {
                actualStrings.add(new String(warehaus.getView(id, timestamp, token).getPacket()));
            }
        }
        //Make sure the two views are the same value
        assertEquals(expectedStrings, actualStrings);

        //Now we add a third view, but set it to squash the old ones
        id.setSquashPrevious(true);
        warehaus.insertView(ByteBuffer.wrap("show me that instead".getBytes()), id, visibility, token);
        Thread.sleep(1000);
        timestamps = warehaus.getVersions(id.getUri(), token);
        timestamps.remove(ts);
        //Assert that we get one view back
        assertEquals(1, timestamps.size());
        assertEquals("show me that instead", new String(warehaus.getView(id, timestamps.get(0), token).getPacket()));
    }
   ...
   public void purgeStatus_PurgeIdNotFound() throws TException {
    ...
    public void testSerialization() throws TException, IOException, ClassNotFoundException {
        ...
        Value val = VisibilitySerialization.serializeVisibilityWithDataToValue(
                new Visibility().setFormalVisibility("U"), vc);
    ...
    public void testPut_ExternalCommunityMarkings() throws TException, InterruptedException {
        ...
        PutUpdateEntry putEntry = new PutUpdateEntry();
        putEntry.setEntry(entry);
        Visibility withCommunity = new Visibility().setFormalVisibility("U").setAdvancedMarkings(
                new AdvancedMarkings().setExternalCommunityVisibility("TEST"));
