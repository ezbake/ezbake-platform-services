...
public class AccumuloDatabaseSetup implements DatabaseSetup {
    ...
    public List<ArtifactDataEntry> setupDatabase(DeploymentArtifact artifact, Properties configuration, EzSecurityToken callerToken) throws DeploymentException {
        ...
        try {
            ...
            // in the user's auths.
            EzSecurityToken token = securityClient.fetchDerivedTokenForApp(callerToken, pool.getSecurityId(EzSecurityServicesConstants.SECURITY_SERVICE_NAME));
            Set<String> authorizations = securityClient.getClient().getAuthorizations(token, TokenType.APP, ArtifactHelpers.getSecurityId(artifact));
            ...
            logger.error("Failed to set Accumulo authorizations for user. Could not retrieve data from Security service", e);
