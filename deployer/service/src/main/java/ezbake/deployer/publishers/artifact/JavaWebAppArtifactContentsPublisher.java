...
public class JavaWebAppArtifactContentsPublisher implements ArtifactContentsPublisher {
    private static final Logger logger = LoggerFactory.getLogger(JavaWebAppArtifactContentsPublisher.class);
    ...
    private Collection<ArtifactDataEntry> collectConfigurationResources(
            DeploymentArtifact artifact, String resourceClassPath, Path artifactBasePath,
    ...
    private Collection<ArtifactDataEntry> collectConfigurationResources(
        ...
        List<ArtifactDataEntry> artifactCollection = Lists.newArrayList();
        final List<String> resources = getResourcesFromClassPath(JavaWebAppArtifactContentsPublisher.class,
                                                                 resourceClassPath);
        final Path resourceDir = Paths.get(resourceClassPath.replaceAll("\\.", "/"));

        for (String resource : resources) {
            try {
                Path artifactPath = Paths.get(artifactBasePath.toString(),
                        resourceDir.relativize(Paths.get(resource)).toString());
    ...
    private boolean doesResourceAlreadyExistInArtifact(Path artifactPath, Path artifactBasePath,
                                                       byte[] artifact) throws  DeploymentException {
        try (TarArchiveInputStream tais = new TarArchiveInputStream(
                new GzipCompressorInputStream(new ByteArrayInputStream(artifact)))) {

            ArchiveEntry nextEntry;
            while ((nextEntry = tais.getNextEntry()) != null) {
                if (nextEntry.isDirectory()) continue;

                Path entryPath = Paths.get(nextEntry.getName());
