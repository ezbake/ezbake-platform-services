...
public class EzBakeDeployerHandlerTest {
    ...
    private static final EzSecurityToken token =
            ThriftTestUtils.generateTestSecurityToken("deployer", "deployer", Lists.newArrayList("U"));
    ...
    private static void assertDeploymentArtifactInCapture(
            Capture<DeploymentArtifact> artifactCapture, String serviceName, String expectedAppId,
    ...
    public void testDeployingOfThriftService() throws Exception {
        ...
        EzBakeDeployerHandler mdh = new EzBakeDeployerHandler(
                publisher, store, ezDeployerConfiguration, new EzbakeSecurityClient(configuration), null, null);
        ...
        assertDeploymentArtifactInCapture(
                artifactCapture, "fieldSearchAppId", "foobarAppId", "fieldSearchAppId");
        assertDeploymentArtifactInCapture(
                artifactCapture, "geoSearchAppId", "foobarAppId", "geoSearchAppId");
    }
    ...
    public void testInjection() throws Exception {
        ...
        try {
            Injector injector =
                    Guice.createInjector(new DeployerModule(), new FakeDeployerModule(), new ConfigurationModule());
    ...
    public void testMultiBindingDatabases() throws Exception {
        ...
        try {
            ...
            ZookeeperConfigurationHelper zooConfig =
                    new ZookeeperConfigurationHelper(injector.getInstance(Properties.class));
    ...
    private static class FakePublisher implements EzPublisher {
        @Override
    ...
    private static class FakeCertsService implements SSLCertsService {
        @Override
    ...
    private static class FakeDeployerModule extends AbstractModule {
        @Override
        protected void configure() {
            bind(SSLCertsService.class).to(FakeCertsService.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.Thrift.class).to(FakePublisher.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.WebApp.class).to(FakePublisher.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.DataSet.class).to(EzDataSetPublisher.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.Custom.class).to(FakePublisher.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.Frack.class).to(FakePublisher.class);
            bind(EzPublisher.class).annotatedWith(EzPublisherMapping.Batch.class).to(FakePublisher.class);
