...
public class TestSecurityHandler extends EzSecurityBaseTest {
    ...
    @Test
    public void testGetAuthsWithCommunityAuths() throws Exception {
        TokenRequest request = new TokenRequest("EzPy", System.currentTimeMillis(), TokenType.APP);
        EzSecurityToken token =
                handler.requestToken(request, EzSecurityTokenBaseTest.getRequestSignature(request, crypto));
        Set<String> expectedAuths = Sets.newHashSet("42six", "CSC", "USA", "comm1", "comm2", "ezbake", "high");
        Set<String> actualAuths = handler.getAuthorizations(token, TokenType.APP, "SecurityClientTest");
        Assert.assertEquals(expectedAuths, actualAuths);
    }

    @Test
    public void testGetAuthsForUser() throws Exception {
        TokenRequest request = new TokenRequest("EzPy", System.currentTimeMillis(), TokenType.APP);
        EzSecurityToken token =
                handler.requestToken(request, EzSecurityTokenBaseTest.getRequestSignature(request, crypto));
        Set<String> expectedAuths = Sets.newHashSet("42six", "CSC", "USA", "ezbake", "high");
        Set<String> actualAuths = handler.getAuthorizations(token, TokenType.USER, "Jim Bob");
        Assert.assertEquals(expectedAuths, actualAuths);
    }
}
